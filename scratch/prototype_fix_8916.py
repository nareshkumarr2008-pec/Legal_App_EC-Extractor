...
def find_value(text, patterns, flags=re.IGNORECASE):
    for pat in patterns:
...
def test_extract(raw_text, filename=None):
    fields = {}
    ...
    if exec_m:
        ...
        if m_num and raw_d and yr and is_valid_date(raw_d, m_num, yr):
    ...
    fields["registration_date"] = {"value": reg_date or "Not Detected", "confidence": 0.95 if reg_date else 0.0}
    ...
    if vendor_details and any(k in vendor_details.lower() for k in ["power of attorney", "power agent", "power ofattorney"]):
        ...
        if poa_v_m:
            poa_agent = clean_legal_text(poa_v_m.group(1)).strip(',').strip()
    ...
    fields["vendor_details"] = {"value": vendor_details or "Not Detected"}
    if poa_agent:
        fields["poa_agent_details"] = {"value": poa_agent}

    # 3. Purchaser Details & POA
    ...
    if fav_m:
        ...
        # Check if line right before FAV contains the person's name/honorific (e.g. Mrs SHAIALAJA SANKAR)
        ...
        if p_poa:
            ...
            poa_raw = clean_legal_text(p_poa.group(2)).strip(',').strip()
    ...
    fields["purchaser_details"] = {"value": purchaser_details or "Not Detected"}

    # 4. History / Previous Owner
    prev_owners = []
    ...
    if orig_m:
        po_cand = clean_str(orig_m.group(1)).strip(',').strip()
    ...
    fields["history_previous_owner"] = {"value": prev_owner_val}
    ...
    for pm in pdr_matches:
        ...
        if any(k in ctx.lower() for k in ["power of attorney", "poa", "power agent", "general power", "book 4", "book iv"]):
    ...
    fields["previous_doc_reference"] = {"value": prev_doc_ref}

    # 6. Survey & Paimash Number
    survey = None
    ...
    fields["survey_number"] = {"value": survey or "Not Detected"}

    # 7. Classification & Property Type
    ...
    fields["schedule_property_type"] = {"value": prop_type}
    fields["land_classification"] = {"value": classification}

    # 8. Village / Taluk / District
    ...
    fields["village_taluk_district"] = {"value": vtd or "Not Detected"}
    ...
    if tot_ext_m:
        ...
        extent_total = clean_str(tot_sum_m.group(1))
    else:
        ext_fb = find_value(norm_text, [
            r'(?:பரப்பு|extent|area)[^\n:]*[:\s]+([^\n]+)',
            r'([0-9.]+\s*(?:sq\.?\s*ft|cents?|acres?|grounds?|ஏர்|ares|hectare))',
        ])
        if ext_fb: extent_total = ext_fb

    fields["land_extent"] = {"value": extent_total or "Not Detected"}
    ...
    flat_desc = None
    fl_m = re.search(r'(Flat\s*No\.?\s*[A-Za-z0-9-]+[^\n\.;]+?(?:(?:Ground|First|Second|Third|Fourth|\d+(?:st|nd|rd|th))\s*Floor)?[^\n\.;]+?(?:Chennai\s*\d{6}|[A-Za-z0-9\s]+Twins|[A-Za-z0-9\s]+Apartments?|[A-Za-z0-9\s]+Enclave|Floor))', sched_scope, re.IGNORECASE)
    if fl_m:
    ...
    fields["apartment_uds_floor"] = {"value": " | ".join(combined_uds) if combined_uds else (uds_val or "Not Detected")}
    ...
    fields["boundaries"] = {"value": boundaries_str or "Not Detected"}

    # 12. Consideration & Market Value
    ...
    fields["consideration_amount"] = {"value": amt_str or "Not Detected"}
    fields["market_value"] = {"value": mv_str or (amt_str or "Not Detected")}
    ...
    fields["payment_breakdown"] = {"value": " | ".join(pay_parts) if pay_parts else "Not Detected"}
    ...
    fields["sro_details"] = {"value": sro}

    # 15. Document Number (Filter out Book 4 POA numbers!)
    doc_no = None
    ...
    if not doc_no:
        # Search all document number stamps/occurrences
        doc_candidates = []
        for dm in re.finditer(r'(?:DOCUMENT|Doc(?:ument)?|DCCUMEN,?|JOCOMENT)[\s\S]{0,40}?(?:No\.?)\s*[:\s]*([0-9A-Za-z\.]+)\s*(?:[\.,\s]*(?:Year|of|oF)\s*(\d{4})|(?:\s*Year\s*(\d{4})))?', norm_text, re.I):
    ...
    fields["document_number"] = {"value": doc_display}

    # 16. Utility & Municipal Identifiers
    tneb = find_value(norm_text, [r'TNEB\s*Service\s*(?:connection)?\s*No\.?\s*([0-9A-Za-z-]+)', r'மின்\s*இணைப்பு\s*எண்\s*[:\s]*([0-9A-Za-z-]+)'])
    cmwssb = find_value(norm_text, [r'CMWSSB\s*customer\s*ID\s*No\.?\s*([0-9A-Za-z]+)', r'குடிநீர்\s*இணைப்பு\s*[:\s]*([0-9A-Za-z]+)'])
    tax_door = find_value(norm_text, [r'property\s*tax\s*assessment\s*([^\n,]+(?:Zone[^\n\)]+\))?)', r'சொத்து\s*வரி\s*[:\s]*([^\n]+)'])
    ...
    fields["utility_tax_identifiers"] = {"value": " | ".join(util_parts) if util_parts else "Not Detected"}

    # 17. Witnesses & Drafter
    wits = []
    ...
    if len(wits) < 2:
        ...
        if id_m:
            ...
            for l in id_lines:
                ...
                if m_name:
                    w_cand = clean_str(m_name.group(1)).replace("Slo. ", "").replace("S/o. ", "").strip()
    ...
    fields["witnesses"] = {"value": witnesses_val}
    ...
    if dr_m:
        ...
        if name_m:
            name_cand = clean_str(name_m.group(1) or name_m.group(2)).strip()
    ...
    fields["document_drafter"] = {"value": drafter or "Self-Drafted / Legal Counsel (Registered at SRO)"}
    ...
    checklist = [
        ...
        },
        {
            ...
            "is_valid": bool(classification and classification != "Not Detected"),
        ...
        },
        {
            ...
            "details": f"Unit Details: {flat_desc or 'Identified Apartment'} | Built-Up: {built_val or 'Specified area'}" if has_flat else f"Land Ownership: Extent {uds_val or extent_total} conveyed"
        },
        {
...
for name, path, fn in deeds:
    ...
    print(f"=======================================================")
    res = test_extract(text, filename=fn)
    
    print(f"1. Reg Date:      {res.get('registration_date', {}).get('value')}")
    print(f"2. Vendor:        {res.get('vendor_details', {}).get('value')[:75]}...")
    print(f"3. Purchaser:     {res.get('purchaser_details', {}).get('value')[:75]}...")
    if 'poa_agent_details' in res:
        print(f"   POA Agent:     {res.get('poa_agent_details', {}).get('value')[:75]}...")
