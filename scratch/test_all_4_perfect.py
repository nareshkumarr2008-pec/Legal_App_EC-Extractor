...
def clean_poa_name(raw: str) -> str:
    ...
    if corp_m:
        ...
        comp = re.sub(r'^[Hh]/s\.', 'M/s.', comp)
        comp = re.sub(r'^[Hh]is\.', 'M/s.', comp)
...
def extract_all_refined(text, fn=None):
    ...
    if vendor_details and any(k in vendor_details.lower() for k in ["power of attorney", "power agent", "power ofattorney"]):
        ...
        if poa_v_m:
            ...
            v_ctx = norm_text[poa_v_m.start():poa_v_m.end() + 450]
    ...
    mother_docs = []
    pdr_matches = re.finditer(r'(?:(?:registered\s+as\s+|vide\s+)?(?:Doc\.?\s*No\.?|Document\s*No\.?|Doc\.No\.|ஆவண\s*எண்)\s*[:\s]*(\d{1,5})[\.,\s]*(?:of|/|\s+of\s+)\s*(\d{2,4}))', norm_text, re.IGNORECASE)
    ...
    prev_owners = []
    clean_p_text = clean_str(re.sub(r'[^\x00-\x7F]+', ' ', norm_text))
    
    # Check 1: "having Purchased ... from [POA] ... and Power Agent of [Owners]" (2004)
    po_m1 = re.search(r'purchased[^\n]+?from\s+([A-Za-z0-9\s\.,&\'\(\)/-]+?),\s*and\s+Power\s+Agent\s+of\s+([0-9A-Za-z\s\.,&\'\(\)/-]+?)(?=,\s*in\s+and|\s*in\s+and|\.\s|\Z)', clean_p_text, re.I)
    if po_m1:
        poa_clean = clean_poa_name(po_m1.group(1))
        owners_clean = clean_str(po_m1.group(2)).strip(',').strip()
        prev_owners.append(f"{owners_clean} (Represented by POA: {poa_clean})")

    # Check 2: "purchased by Vendor from [Owners] represented by General Power of Attorney Agent [POA]" (2010 Naagesh)
    if not prev_owners:
        po_m2 = re.search(r'purchased\s+by\s+the\s+Vendor\s+herein[^\n]+?from\s+(.+?)\s+represented\s+by\s+their\s+(?:General\s+)?Power\s+of\s+Attorney\s+Agent\s+(.+?)(?=,\s*and\s+the\s+same|,\s*and\s+registered|\.\s|\Z)', clean_p_text, re.I)
        if po_m2:
            owners_clean = clean_str(po_m2.group(1)).replace(' I ', ' ').strip()
            poa_clean = clean_poa_name(po_m2.group(2))
            prev_owners.append(f"{owners_clean} (Represented by POA: {poa_clean})")

    # Check 3: "originally owned by one [Owner], he had purchased ... from [Prior Owner]" (2010 Shailaja)
    if not prev_owners:
        po_m3 = re.search(r'originally\s+owned\s+by\s+(?:one\s+)?([A-Za-z0-9\.\s,\(\)\'’/&:-]+?)(?:,\s*he\s+had\s+purchased|,\s*who\s+purchased|\.\s|\Z)', clean_p_text, re.I)
        if po_m3:
            c_cand = clean_str(po_m3.group(1)).strip(',').strip()
            if len(c_cand) > 3:
                prev_owners.append(c_cand)
                pur_sub = re.search(r'purchased\s+(?:fhe|the)\s+said\s+properties\s+from\s+([A-Za-z0-9\.\s,\(\)\'’/&:-]+?)(?:under\s+the\s+Deed|under\s+a\s+registered|,\s*in\s+and\s+by|\Z)', clean_p_text, re.I)
                if pur_sub:
                    prev_owners.append(f"Purchased from {clean_str(pur_sub.group(1)).strip(',').strip()}")

    # Check 4: "purchased ... from one [Owner] on [Date]" (1995)
    if not prev_owners:
        po_m4 = re.search(r'purchased\s+(?:the\s+property\s+)?from\s+(?:one\s+)?([A-Za-z\.\s]+?)\s+on\s+([0-9A-Za-z\s]+?)(?:under\s+a|\s+under|,\s*and|\.\s|\Z)', clean_p_text, re.I)
        if po_m4:
            prev_owners.append(f"Purchased from {clean_str(po_m4.group(1))} on {clean_str(po_m4.group(2))}")
...
for name, path, fn in deeds:
    with open(path, 'r', encoding='utf-8') as f:
        text = f.read()
    res = extract_all_refined(text, fn)
    print(f"\n=======================================================")
